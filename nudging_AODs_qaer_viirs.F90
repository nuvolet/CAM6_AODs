...
!   real(r8), allocatable :: mlp_c(:,:,:)     ! (ncol,pver,lchnk)   midpoint pressures computed    
  ...
  real(r8),allocatable:: Computed_AOD(:,:)      ! pcols,begchunk:endchunk  
  real(r8),allocatable:: AOD_ratio(:,:)   !pcols,begchunk:endchunk ratio between MODIS_AOD/computed_AOD
...
!    call alloc_err(istat,'nudging_init_AOD','mlp_c',pcols*((endchunk-begchunk)+1))
   ...
   ! --- Allocate space for Computed_AOD -------------------------------
   allocate(Computed_AOD(pcols,begchunk:endchunk),stat=istat)
   call alloc_err(istat,'nudging_init_AOD','Computed_AOD',pcols*((endchunk-begchunk)+1))
...
!    call addfld('mlp_c',(/ 'lev ' /),'A','Pa'  ,'Mid-point pressure computed ', flag_xyfill=.true.,fill_value=-999.0_r8)    
   ...
   call addfld ('Computed_AOD',   horiz_only, 'A','  ','Computed AOD',  flag_xyfill=.true.,fill_value=-999.0_r8)      
   call addfld ('tauxar',(/ 'lev '/),'A',' '  ,'Layer extinction optical depth  ', flag_xyfill=.true.,fill_value=-999.0_r8)
           
   call addfld ('AOD_ratio',   horiz_only, 'A','  ','Nudging scale factor MODIS/Computed',  flag_xyfill=.true.,fill_value=-999.0_r8)                    
...
!         call calc_MidpointPressure(lchnk,ncol)
	    ...
	    call aerosol_sw_properties(lchnk,ncol,ncoef,phys_state(lchnk)) 
...
! 		   ! Contrarily what it was expected, declaring Target_Qaer is filled and updated in qaer_target_mam4 subroutine, there fore it is unnecessary to create 
...
!       call outfld('palb',          palb(:,lchnk),    pcols, lchnk)           
      ...
      call outfld('Computed_AOD',  Computed_AOD(:,lchnk),    pcols, lchnk)            
...
!      write(iulog,*) 'ERROR: nudging_update_analyses_fv: plev=',plev,' pver=',pver
	...
	! NOTE here it reads the variable in the netCDF file for the AOD
     ...
     istat=nf90_inq_varid(ncid,'aod_weighted_mean_viirs',varid)	
     if(istat.ne.NF90_NOERR) then
       write(iulog,*) nf90_strerror(istat)
       call endrun ('UPDATE_ANALYSES_FV')
     endif
...
! !   ================================================================ 
  ...
  subroutine relative_humidity(lchnk, ncol, pver, q, t, pmid)
    ...
    real(r8), intent(in) :: q(:,:)     ! (ncol,pver) specific humidity
...
!     real(r8), intent(out) :: RH(:,:,:)  ! relative humidity

    real, parameter :: eps = 0.622    ! R_d / R_v
    real, parameter :: e_s_0 = 611.2  ! saturation vapor pressure at 0 C

!     RH(:,:,lchnk) = 0.8

    ! --- variables to compute RH from q & t
        
    ! compute RH from Q and T
    ! ------ compute RH from q and t                       
    ! ---- From D. Fillmore

    RH(:,:,lchnk) = pmid(:,:) * q(:,:) &
      / (eps * e_s_0 * exp(17.67 * (t(:,:) - 273.15) / (t(:,:)  -  29.65)))
   
    do i = 1,ncol
     do j = 1, pver
      
	    if (RH(i,j,lchnk).gt.1) then  
			RH(i,j,lchnk) = 1
	    end if
  ...
  subroutine Create_Model_Qaer_Mode(ncol,pver,lchnk)	
    ...
    ! Modal Aerosol Model 4
    ! Accumulation: Su, POM, SOA x 5, BC, Du, SS, Num
    ! Aitken: Su, SOA x 5, Du, SS, Num
    ! Coarse: Su, Du, SS, Num
    ! Primary Carbon: POM, BC, Num

    integer :: i_accum_su, i_accum_pom, i_accum_soa, i_accum_bc, &
               i_accum_du, i_accum_ss, i_accum_num
    integer :: i_aitken_su, i_aitken_soa, &
               i_aitken_du, i_aitken_ss, i_aitken_num
    integer :: i_coarse_su, i_coarse_du, i_coarse_ss, i_coarse_num
    integer :: i_primary_c_pom, i_primary_c_bc, i_primary_c_num, i, j
    ...
    ! --------> accumulation mode
     Model_Qaer_Mode(:,:,1,:) = &
      + Model_Qaer(:,:,i_accum_su,:)  &
      + Model_Qaer(:,:,i_accum_pom,:) &
      + Model_Qaer(:,:,i_accum_soa,:) &
      + Model_Qaer(:,:,i_accum_soa + 1,:)  &
      + Model_Qaer(:,:,i_accum_soa + 2,:)  &
      + Model_Qaer(:,:,i_accum_soa + 3,:)  &
      + Model_Qaer(:,:,i_accum_soa + 4,:)  &
      + Model_Qaer(:,:,i_accum_bc,:)  &
      + Model_Qaer(:,:,i_accum_du,:)  &
      + Model_Qaer(:,:,i_accum_ss,:) 

!      Model_Qaer_Mode(:,:,1,lchnk) = &
!       Model_Qaer(:,:,i_accum_su,lchnk) 
      
    ! --------> aitken mode      
    Model_Qaer_Mode(:,:,2,:) = &
      + Model_Qaer(:,:,i_aitken_su,:)  &
      + Model_Qaer(:,:,i_aitken_soa,:) &
      + Model_Qaer(:,:,i_aitken_soa + 1,:)  &
      + Model_Qaer(:,:,i_aitken_soa + 2,:)  &
      + Model_Qaer(:,:,i_aitken_soa + 3,:)  &
      + Model_Qaer(:,:,i_aitken_soa + 4,:)  &
      + Model_Qaer(:,:,i_aitken_du,:)  & 
      + Model_Qaer(:,:,i_aitken_ss,:) 
      
    ! --------> coarse mode      
    Model_Qaer_Mode(:,:,3,:) = &
      + Model_Qaer(:,:,i_coarse_su,:)  &
      + Model_Qaer(:,:,i_coarse_du,:)  &
      + Model_Qaer(:,:,i_coarse_ss,:) 

    ! --------> primary carbon mode      
    Model_Qaer_Mode(:,:,4,:) = &
      + Model_Qaer(:,:,i_primary_c_pom,:) &
      + Model_Qaer(:,:,i_primary_c_bc,:) 
      
    return
  end subroutine !Create_Model_Qaer_Mode
  ...
  subroutine water_uptake_mam4(ncol,pver,lchnk)  
    ...
    ! Compute bulk hygroscopicity and wet modal radius for each mode
    ! from GZ07 Eq 5, Eq 3, and Eq 2

   use shr_const_mod ,only: SHR_CONST_PI ! pi constant
...
!    integer, parameter :: ncoef=5, prefr=7, prefi=10
   ...
   integer :: list_idx=0  ! index of the climate or a diagnostic list   
    ...
    ! Parameters
    ! ----------
    integer, parameter :: nmde = 4   ! number of modes MAM4 (Accumulation, Aitken, Coarse, Primary Carbon) 
    
    ! geometric standard deviation
    real, parameter :: gsd(4) = (/ 1.6, 1.6, 1.2, 1.600000024 /)  
    
    ! refractive index for each component @ 550 nm

    real, parameter :: n_water(2) = (/ 1.333, 1.96e-9 /)
    real, parameter :: n_su(2) = (/ 1.43, 1.000e-08 /)
    real, parameter :: n_pom(2) = (/ 1.53, 0.006 /)
    real, parameter :: n_so(2) = (/ 1.53, 0.006 /)
    real, parameter :: n_bc(2) = (/ 1.75, 0.44 /)
    real, parameter :: n_du(2) = (/ 1.22000002, 0.021 /)
    real, parameter :: n_ss(2) = (/ 1.5, 9.99999994e-9 /)
        
    ! dry mass densities (kg m-3)
    real, parameter :: rho_su  = 1.7e3
    real, parameter :: rho_pom = 1.8e3
    real, parameter :: rho_soa = 1.8e3
    real, parameter :: rho_bc  = 1.0e3
    real, parameter :: rho_du  = 2.6e3
    real, parameter :: rho_ss  = 2.2e3

    real, parameter :: Bcoeff_su(3)  = (/ 2.42848, -3.85261, 1.88159 /)
    real, parameter :: Bcoeff_pom(3) = (/ 0.14, 0.0, 0.0 /)
    real, parameter :: Bcoeff_soa(3) = (/ 0.14, 0.0, 0.0 /)
    real, parameter :: Bcoeff_bc(3)  = (/ 0.01, 0.0, 0.0 /)
    real, parameter :: Bcoeff_du(3)  = (/ 0.14, 0.0, 0.0 /)
    real, parameter :: Bcoeff_ss(3)  = (/ 4.83257, -6.92329, 3.27805 /)

!     real (r8), parameter :: vmr_cutoff = 1.0e-10    
    real (r8), parameter :: vmr_cutoff = 1.0e-20    
    real (r8), parameter :: Bmode_zero = 0.00

    real (r8), parameter :: RH_cutoff = 0.99
    real (r8), parameter :: rwet_zero = 0.00    

    
!     real, allocatable :: vmr(:,:)
    real, allocatable :: B_su(:,:)
    real, allocatable :: B_pom(:,:)
    real, allocatable :: B_soa(:,:)
    real, allocatable :: B_bc(:,:)
    real, allocatable :: B_du(:,:)
    real, allocatable :: B_ss(:,:)
    ...
    allocate(B_su(ncol,pver))
    allocate(B_pom(ncol,pver))
    allocate(B_soa(ncol,pver))
    allocate(B_bc(ncol,pver))
    allocate(B_du(ncol,pver))
    allocate(B_ss(ncol,pver))

    ! GZ07 Eq 5
    B_su(:,:) &
      = Bcoeff_su(1) + Bcoeff_su(2) * RH(:,:,lchnk) + Bcoeff_su(3) * RH(:,:,lchnk)**2      
    B_pom(:,:) &
      = Bcoeff_pom(1) + Bcoeff_pom(2) * RH(:,:,lchnk) + Bcoeff_pom(3) * RH(:,:,lchnk)**2
    B_soa(:,:) &
      = Bcoeff_soa(1) + Bcoeff_soa(2) * RH(:,:,lchnk) + Bcoeff_soa(3) * RH(:,:,lchnk)**2
    B_bc(:,:) &
      = Bcoeff_bc(1) + Bcoeff_bc(2) * RH(:,:,lchnk) + Bcoeff_bc(3) * RH(:,:,lchnk)**2
    B_du(:,:) &
      = Bcoeff_du(1) + Bcoeff_du(2) * RH(:,:,lchnk) + Bcoeff_du(3) * RH(:,:,lchnk)**2

    B_ss(:,:) &
      = Bcoeff_ss(1) + Bcoeff_ss(2) * RH(:,:,lchnk) + Bcoeff_ss(3) * RH(:,:,lchnk)**2
    ...
    ! --------> accumulation mode
     ...
     vmr(:,:,1,lchnk) = &
      + Model_Qaer(:,:,i_accum_su,lchnk) / rho_su &
      + Model_Qaer(:,:,i_accum_pom,lchnk) / rho_pom &
      + Model_Qaer(:,:,i_accum_soa,lchnk) / rho_soa &
      + Model_Qaer(:,:,i_accum_soa + 1,lchnk) / rho_soa &
      + Model_Qaer(:,:,i_accum_soa + 2,lchnk) / rho_soa &
      + Model_Qaer(:,:,i_accum_soa + 3,lchnk) / rho_soa &
      + Model_Qaer(:,:,i_accum_soa + 4,lchnk) / rho_soa &
      + Model_Qaer(:,:,i_accum_bc,lchnk) / rho_bc &
      + Model_Qaer(:,:,i_accum_du,lchnk) / rho_du &
      + Model_Qaer(:,:,i_accum_ss,lchnk) / rho_ss

    B_mode(:,:,1,lchnk) = &
      + B_su * Model_Qaer(:,:,i_accum_su,lchnk) / rho_su &
      + B_pom * Model_Qaer(:,:,i_accum_pom,lchnk) / rho_pom &
      + B_soa * Model_Qaer(:,:,i_accum_soa,lchnk) / rho_soa &
      + B_soa * Model_Qaer(:,:,i_accum_soa + 1,lchnk) / rho_soa &
      + B_soa * Model_Qaer(:,:,i_accum_soa + 2,lchnk) / rho_soa &
      + B_soa * Model_Qaer(:,:,i_accum_soa + 3,lchnk) / rho_soa &
      + B_soa * Model_Qaer(:,:,i_accum_soa + 4,lchnk) / rho_soa &
      + B_bc * Model_Qaer(:,:,i_accum_bc,lchnk) / rho_bc &
      + B_du * Model_Qaer(:,:,i_accum_du,lchnk) / rho_du &
      + B_ss * Model_Qaer(:,:,i_accum_ss,lchnk) / rho_ss

    B_mode(:,:,1,lchnk) = merge(B_mode(:,:,1,lchnk) / vmr(:,:,1,lchnk), &
      Bmode_zero, vmr(:,:,1,lchnk) > vmr_cutoff)
      
    ! --------> aitken mode
    vmr(:,:,2,lchnk) = &
      + Model_Qaer(:,:,i_aitken_su,lchnk) / rho_su &
      + Model_Qaer(:,:,i_aitken_soa,lchnk) / rho_soa &
      + Model_Qaer(:,:,i_aitken_soa + 1,lchnk) / rho_soa &
      + Model_Qaer(:,:,i_aitken_soa + 2,lchnk) / rho_soa &
      + Model_Qaer(:,:,i_aitken_soa + 3,lchnk) / rho_soa &
      + Model_Qaer(:,:,i_aitken_soa + 4,lchnk) / rho_soa &
      + Model_Qaer(:,:,i_aitken_du,lchnk) / rho_du &
      + Model_Qaer(:,:,i_aitken_ss,lchnk) / rho_ss

    B_mode(:,:,2,lchnk) = &
      + B_su * Model_Qaer(:,:,i_aitken_su,lchnk) / rho_su &
      + B_soa * Model_Qaer(:,:,i_aitken_soa,lchnk) / rho_soa &
      + B_soa * Model_Qaer(:,:,i_aitken_soa + 1,lchnk) / rho_soa &
      + B_soa * Model_Qaer(:,:,i_aitken_soa + 2,lchnk) / rho_soa &
      + B_soa * Model_Qaer(:,:,i_aitken_soa + 3,lchnk) / rho_soa &
      + B_soa * Model_Qaer(:,:,i_aitken_soa + 4,lchnk) / rho_soa &
      + B_du * Model_Qaer(:,:,i_aitken_du,lchnk) / rho_du &
      + B_ss * Model_Qaer(:,:,i_aitken_ss,lchnk) / rho_ss

    B_mode(:,:,2,lchnk) = merge(B_mode(:,:,2,lchnk) / vmr(:,:,2,lchnk), &
      Bmode_zero, vmr(:,:,2,lchnk) > vmr_cutoff)
      

    ! --------> coarse mode
    vmr(:,:,3,lchnk) = &
      + Model_Qaer(:,:,i_coarse_su,lchnk) / rho_su &
      + Model_Qaer(:,:,i_coarse_du,lchnk) / rho_du &
      + Model_Qaer(:,:,i_coarse_ss,lchnk) / rho_ss

    B_mode(:,:,3,lchnk) = &
      + B_su * Model_Qaer(:,:,i_coarse_su,lchnk) / rho_su &
      + B_du * Model_Qaer(:,:,i_coarse_du,lchnk) / rho_du &
      + B_ss * Model_Qaer(:,:,i_coarse_ss,lchnk) / rho_ss

    B_mode(:,:,3,lchnk) = merge(B_mode(:,:,3,lchnk) / vmr(:,:,3,lchnk), &
      Bmode_zero, vmr(:,:,3,lchnk) > vmr_cutoff)
      

    ! --------> primary carbon mode
    vmr(:,:,4,lchnk) = &
      + Model_Qaer(:,:,i_primary_c_pom,lchnk) / rho_pom &
      + Model_Qaer(:,:,i_primary_c_bc,lchnk) / rho_bc

    B_mode(:,:,4,lchnk) = &
      + B_pom * Model_Qaer(:,:,i_primary_c_pom,lchnk) / rho_pom &
      + B_bc * Model_Qaer(:,:,i_primary_c_bc,lchnk) / rho_bc

    B_mode(:,:,4,lchnk) = merge(B_mode(:,:,4,lchnk) / vmr(:,:,4,lchnk), &
      Bmode_zero, vmr(:,:,4,lchnk) > vmr_cutoff)
      
    ! --- Computing the Wet Surface Mode Radius GZ07 Eq 2
    !
    ! ----------------------------->Dry surface mode radius GZ07 Eq 1
    !--------> accumulation mode    
    rdry_mode(:,:,1,lchnk) = ((((3/(4 * SHR_CONST_PI * Model_Qaer(:,:,i_accum_num,lchnk))) * vmr(:,:,1,lchnk)))**(1./3.))*exp((1./2.)*(log10(gsd(1)))**2)   
    !--------> Aitken mode    
    rdry_mode(:,:,2,lchnk) = ((((3/(4 * SHR_CONST_PI * Model_Qaer(:,:,i_aitken_num,lchnk))) * vmr(:,:,2,lchnk)))**(1./3.))*exp((1./2.)*(log10(gsd(2)))**2)   
    !--------> coarse mode    
    rdry_mode(:,:,3,lchnk) = ((((3/(4 * SHR_CONST_PI * Model_Qaer(:,:,i_coarse_num,lchnk))) * vmr(:,:,3,lchnk)))**(1./3.))*exp((1./2.)*(log10(gsd(3)))**2)   
    !--------> Primary Carbon  mode    
    rdry_mode(:,:,4,lchnk) = ((((3/(4 * SHR_CONST_PI * Model_Qaer(:,:,i_primary_c_num,lchnk))) * vmr(:,:,4,lchnk)))**(1./3.))*exp((1./2.)*(log10(gsd(4)))**2)   
    
    ! -----------------------------> Wet surface mode radius GZ07 Eq 2    
    !     ln(RH) =(2M_W*Sigma/R*T*rho__w*r_wet) -(B_mode*R_d**3/(r_w**3-R_d**3))
    !     Assuming we can neglet first term: (2M_W*Sigma/R*T*rho__w*r_wet) ~ 0
    !     ln(RH) =-(B_mode*R_d**3/(r_w**3-R_d**3))
    !     r_wet =[r_d**3 - (B*r_d**3)/(ln(RH)))]**(1/3))
    !     r_wet =r_d*[1-B/ln(RH)]**(1/3)
        
    !--------> accumulation mode    
    
    Brd(:,:,1,lchnk) =   merge(B_mode(:,:,1,lchnk) / log(RH(:,:,lchnk)), &
    B_mode(:,:,1,lchnk) / log(RH_cutoff), RH(:,:,lchnk) <= RH_cutoff)
        
	rwet_mode(:,:,1,lchnk) =  rdry_mode(:,:,1,lchnk) *(1 - Brd(:,:,1,lchnk))**(1./3.)    
	        
    !--------> Aitken mode    

    Brd(:,:,2,lchnk) =   merge(B_mode(:,:,2,lchnk) / log(RH(:,:,lchnk)), &
     B_mode(:,:,2,lchnk) / log(RH_cutoff),  RH(:,:,lchnk) <= RH_cutoff)
        
	rwet_mode(:,:,2,lchnk) =  rdry_mode(:,:,2,lchnk) *(1 - Brd(:,:,2,lchnk))**(1./3.)    
        
    !--------> coarse mode    

    Brd(:,:,3,lchnk) =   merge(B_mode(:,:,3,lchnk) / log(RH(:,:,lchnk)), &
     B_mode(:,:,3,lchnk) / log(RH_cutoff),  RH(:,:,lchnk) <= RH_cutoff)
	rwet_mode(:,:,3,lchnk) =  rdry_mode(:,:,3,lchnk) *(1 - Brd(:,:,3,lchnk))**(1./3.)    
            
    !--------> Primary Carbon  mode    
    
    Brd(:,:,4,lchnk) =   merge(B_mode(:,:,4,lchnk) / log(RH(:,:,lchnk)), &
     B_mode(:,:,4,lchnk) / log(RH_cutoff), RH(:,:,lchnk) <= RH_cutoff)
        
	rwet_mode(:,:,4,lchnk) =  rdry_mode(:,:,4,lchnk) *(1 - Brd(:,:,4,lchnk))**(1./3.)    
        
    !!--- Optical Properties (Eq. 11, 12, 13 and 14)
    
    !--- vmr_wet (Volume Mixing Ratio water):
    !--------> accumulation mode        
    vmr_wet(:,:,1,lchnk) = vmr(:,:,1,lchnk) * (rwet_mode(:,:,1,lchnk) /rdry_mode(:,:,1,lchnk) )**3
    vmr_water(:,:,1,lchnk) = vmr_wet(:,:,1,lchnk) - vmr(:,:,1,lchnk)
    !--------> Aitken mode 
    vmr_wet(:,:,2,lchnk) = vmr(:,:,2,lchnk) * (rwet_mode(:,:,2,lchnk) /rdry_mode(:,:,2,lchnk) )**3
    vmr_water(:,:,2,lchnk) = vmr_wet(:,:,2,lchnk) - vmr(:,:,2,lchnk)
    !--------> coarse mode   
    vmr_wet(:,:,3,lchnk) = vmr(:,:,3,lchnk) * (rwet_mode(:,:,3,lchnk) /rdry_mode(:,:,3,lchnk) )**3
    vmr_water(:,:,3,lchnk) = vmr_wet(:,:,3,lchnk) - vmr(:,:,3,lchnk)
    !--------> Primary Carbon  mode  
    vmr_wet(:,:,4,lchnk) = vmr(:,:,4,lchnk) * (rwet_mode(:,:,4,lchnk) /rdry_mode(:,:,4,lchnk) )**3
    vmr_water(:,:,4,lchnk) = vmr_wet(:,:,4,lchnk) - vmr(:,:,4,lchnk)
    
    ! --- Real Wet refractive index for each component and mode:
    !--------> accumulation mode       
    wet_ref_real_index(:,:,1,lchnk) = &
      + n_water(1) * vmr_water(:,:,1,lchnk) &
      + n_su(1) * Model_Qaer(:,:,i_accum_su,lchnk) / rho_su &
      + n_pom(1) * Model_Qaer(:,:,i_accum_pom,lchnk) / rho_pom &
      + n_so(1) * Model_Qaer(:,:,i_accum_soa + 1,lchnk) / rho_soa &
      + n_so(1) * Model_Qaer(:,:,i_accum_soa + 2,lchnk) / rho_soa &
      + n_so(1) * Model_Qaer(:,:,i_accum_soa + 3,lchnk) / rho_soa &
      + n_so(1) * Model_Qaer(:,:,i_accum_soa + 4,lchnk) / rho_soa &
      + n_bc(1) * Model_Qaer(:,:,i_accum_bc,lchnk) / rho_bc &
      + n_du(1) * Model_Qaer(:,:,i_accum_du,lchnk) / rho_du &
      + n_ss(1) * Model_Qaer(:,:,i_accum_ss,lchnk) / rho_ss
     
    wet_ref_real_index(:,:,1,lchnk) = (1/vmr_wet(:,:,1,lchnk)) * wet_ref_real_index(:,:,1,lchnk)

    !--------> Aitken mode       
    wet_ref_real_index(:,:,2,lchnk) = &
      + n_water(1) * vmr_water(:,:,2,lchnk) &
      + n_su(1) * Model_Qaer(:,:,i_aitken_su,lchnk) / rho_su &
      + n_so(1) * Model_Qaer(:,:,i_aitken_soa + 1,lchnk) / rho_soa &
      + n_so(1) * Model_Qaer(:,:,i_aitken_soa + 2,lchnk) / rho_soa &
      + n_so(1) * Model_Qaer(:,:,i_aitken_soa + 3,lchnk) / rho_soa &
      + n_so(1) * Model_Qaer(:,:,i_aitken_soa + 4,lchnk) / rho_soa &
      + n_du(1) * Model_Qaer(:,:,i_aitken_du,lchnk) / rho_du &
      + n_ss(1) * Model_Qaer(:,:,i_aitken_ss,lchnk) / rho_ss
     
    wet_ref_real_index(:,:,2,lchnk) = (1/vmr_wet(:,:,2,lchnk)) * wet_ref_real_index(:,:,2,lchnk)
    
    !--------> Coarse mode       
    wet_ref_real_index(:,:,3,lchnk) = &
      + n_water(1) * vmr_water(:,:,3,lchnk) &
      + n_su(1) * Model_Qaer(:,:,i_coarse_su,lchnk) / rho_su &
      + n_du(1) * Model_Qaer(:,:,i_coarse_du,lchnk) / rho_du &
      + n_ss(1) * Model_Qaer(:,:,i_coarse_ss,lchnk) / rho_ss
     
    wet_ref_real_index(:,:,3,lchnk) = (1/vmr_wet(:,:,3,lchnk)) * wet_ref_real_index(:,:,3,lchnk)
    
    !--------> Primary Carbon mode       
    wet_ref_real_index(:,:,4,lchnk) = &
      + n_water(1) * vmr_water(:,:,4,lchnk) &
      + n_pom(1) * Model_Qaer(:,:,i_primary_c_pom,lchnk) / rho_pom &
      + n_bc(1) * Model_Qaer(:,:,i_primary_c_bc,lchnk) / rho_bc
     
    wet_ref_real_index(:,:,4,lchnk) = (1/vmr_wet(:,:,4,lchnk)) * wet_ref_real_index(:,:,4,lchnk)
    
    
    ! --- Imaginary Wet refractive index for each component and mode:
    !--------> accumulation mode       
    wet_ref_im_index(:,:,1,lchnk) = &
      + n_water(2) * vmr_water(:,:,1,lchnk) &
      + n_su(2) * Model_Qaer(:,:,i_accum_su,lchnk) / rho_su &
      + n_pom(2) * Model_Qaer(:,:,i_accum_pom,lchnk) / rho_pom &
      + n_so(2) * Model_Qaer(:,:,i_accum_soa + 1,lchnk) / rho_soa &
      + n_so(2) * Model_Qaer(:,:,i_accum_soa + 2,lchnk) / rho_soa &
      + n_so(2) * Model_Qaer(:,:,i_accum_soa + 3,lchnk) / rho_soa &
      + n_so(2) * Model_Qaer(:,:,i_accum_soa + 4,lchnk) / rho_soa &
      + n_bc(2) * Model_Qaer(:,:,i_accum_bc,lchnk) / rho_bc &
      + n_du(2) * Model_Qaer(:,:,i_accum_du,lchnk) / rho_du &
      + n_ss(2) * Model_Qaer(:,:,i_accum_ss,lchnk) / rho_ss
     
    wet_ref_im_index(:,:,1,lchnk) = (1/vmr_wet(:,:,1,lchnk)) * wet_ref_im_index(:,:,1,lchnk)

    !--------> Aitken mode       
    wet_ref_im_index(:,:,2,lchnk) = &
      + n_water(2) * vmr_water(:,:,2,lchnk) &
      + n_su(2) * Model_Qaer(:,:,i_aitken_su,lchnk) / rho_su &
      + n_so(2) * Model_Qaer(:,:,i_aitken_soa + 1,lchnk) / rho_soa &
      + n_so(2) * Model_Qaer(:,:,i_aitken_soa + 2,lchnk) / rho_soa &
      + n_so(2) * Model_Qaer(:,:,i_aitken_soa + 3,lchnk) / rho_soa &
      + n_so(2) * Model_Qaer(:,:,i_aitken_soa + 4,lchnk) / rho_soa &
      + n_du(2) * Model_Qaer(:,:,i_aitken_du,lchnk) / rho_du &
      + n_ss(2) * Model_Qaer(:,:,i_aitken_ss,lchnk) / rho_ss
     
    wet_ref_im_index(:,:,2,lchnk) = (1/vmr_wet(:,:,2,lchnk)) * wet_ref_im_index(:,:,2,lchnk)
    
    !--------> Coarse mode       
    wet_ref_im_index(:,:,3,lchnk) = &
      + n_water(2) * vmr_water(:,:,3,lchnk) &
      + n_su(2) * Model_Qaer(:,:,i_coarse_su,lchnk) / rho_su &
      + n_du(2) * Model_Qaer(:,:,i_coarse_du,lchnk) / rho_du &
      + n_ss(2) * Model_Qaer(:,:,i_coarse_ss,lchnk) / rho_ss
     
    wet_ref_im_index(:,:,3,lchnk) = (1/vmr_wet(:,:,3,lchnk)) * wet_ref_im_index(:,:,3,lchnk)
    
    !--------> Primary Carbon mode       
    wet_ref_im_index(:,:,4,lchnk) = &
      + n_water(2) * vmr_water(:,:,4,lchnk)  &
      + n_pom(2) * Model_Qaer(:,:,i_primary_c_pom,lchnk) / rho_pom &
      + n_bc(2) * Model_Qaer(:,:,i_primary_c_bc,lchnk) / rho_bc
     
    wet_ref_im_index(:,:,4,lchnk) = (1/vmr_wet(:,:,4,lchnk)) * wet_ref_im_index(:,:,4,lchnk)    
    ...
    return
  end subroutine ! water_uptake_mam4
  !================================================================
  ...
  subroutine aerosol_sw_properties(lchnk,ncol,ncoef,phys_state)
   ...
   use ref_pres,          only: top_lev => clim_modal_aero_top_lev   
   use radconstants,      only: nswbands, nlwbands, idx_sw_diag, idx_uv_diag, idx_nir_diag   
   use modal_aer_opt ,only: modal_aero_sw, binterp, modal_size_parameters, read_water_refindex ! bilinear interpolation of table
!    use ppgrid,            only: pcols!, pver, pverp    
   use ppgrid,            only: pver, pverp    
   use physconst,         only: rhoh2o, rga, rair
   use spmd_utils,        only: masterproc   
   ...
   !use qaer_target_mam4 ! use module to get wet_ref_real_index and wet_ref_im_index
   
   ! Arguments
   ! ---------
!    integer, intent(in) :: pcols  ! number of columns in chunk
   integer, intent(in) :: ncol  ! number of columns in chunk
   integer, intent(in) :: ncoef  ! number of coefficients
...
!    integer :: nmodes
   ...
   integer :: nspec
   integer :: troplevchem(ncol)       ! Chemical tropopause level
   integer :: istat

   real(r8) :: mass(ncol,pver)        ! layer mass
   real(r8) :: air_density(ncol,pver) ! (kg/m3)
   ...
   real(r8) :: sigma_logr_aer         ! geometric standard deviation of number distribution
   real(r8) :: radsurf(ncol,pver,lchnk)    ! aerosol surface mode radius
   real(r8) :: logradsurf(ncol,pver,lchnk) ! log(aerosol surface mode radius)
   real(r8) :: cheb(ncoef,ncol,pver)
...
!    real(r8) :: wetvol(ncol)   ! volume concentration of wet mode (m3/kg)
   real(r8) :: wetvol(ncol,pver)   ! volume concentration of wet mode (m3/kg)
...
!    real(r8) :: dopaer(ncol,lchnk)   ! aerosol optical depth in layer
   
!    real(r8) :: pabs(ncol)     ! parameterized specific absorption (m2/kg)
!    real(r8) :: pasm(ncol)     ! parameterized asymmetry factor
!    real(r8) :: palb(ncol,lchnk)     ! parameterized single scattering albedo

   integer :: list_idx=0  ! index of the climate or a diagnostic list   
!    integer :: nmodes
!    integer :: nspec

!    print *, 'Number of sw bands: ',nswbands
!    print *, 'Index sw : ',idx_sw_diag
   real(r8) :: rmmin, rmmax       ! min, max aerosol surface mode radius treated (m)
   
   ! --- parameters for equation 14 
   real(r8) :: rwet_mode_min(ncol,pver,ncoef,lchnk)     
   real(r8) :: rwet_mode_max(ncol,pver,ncoef)     
   real(r8) :: xrwet(ncol,pver,ncoef)     
   ...
   Computed_AOD(:,lchnk) = 0._r8   

!    mass(:ncol,:)        = state%pdeldry(:ncol,:)*rga
!    air_density(:ncol,:) = state%pmid(:ncol,:)/(rair*state%t(:ncol,:))
   mass(:ncol,:)        = phys_state%pdeldry(:ncol,:)*rga
   air_density(:ncol,:) = phys_state%pmid(:ncol,:)/(rair*phys_state%t(:ncol,:))
!    wetvol(:ncol,:) = phys_state%q(:ncol,:,1)
...
!                call endrun(routine//': ERROR - file and module values do not match')
   ...
   ! --- using binterp to compute parameterizations ans finally the Computed_AOD    
   do m = 1, nmodes
      ...
      ! --- calc size parameter for all columns --- commented because dgnumwet needs pbuf
      !call modal_size_parameters(ncol, sigma_logr_aer, dgnumwet, radsurf, logradsurf, cheb)

!       do isw = 1, nswbands ! --- loop over all the sw channels 1:14
      do isw = idx_sw_diag,idx_sw_diag ! --- fixed to visible channel
...
!                          itab, jtab, ttab, utab, casm)
           ...
           ! wet_ref_real_index(pcols,pver,nmde,lchnk) ---> ncol,k,m,: : is lchnk
           ! wet_ref_im_index(pcols,pver,nmde,lchnk)   ---> ncol,k,m,: : is lchnk

            call binterp(extpsw(:,:,:,isw), ncol, ncoef, prefr, prefi, &
                         wet_ref_real_index(:,k,m,lchnk), wet_ref_im_index(:,k,m,lchnk), refrtabsw(:,isw), refitabsw(:,isw), &
                         itab, jtab, ttab, utab, cext(:,:,lchnk))
            call binterp(abspsw(:,:,:,isw), ncol, ncoef, prefr, prefi, &
                         wet_ref_real_index(:,k,m,lchnk), wet_ref_im_index(:,k,m,lchnk), refrtabsw(:,isw), refitabsw(:,isw), &
                         itab, jtab, ttab, utab, cabs(:,:,lchnk))
            call binterp(asmpsw(:,:,:,isw), ncol, ncoef, prefr, prefi, &
                         wet_ref_real_index(:,k,m,lchnk), wet_ref_im_index(:,k,m,lchnk), refrtabsw(:,isw), refitabsw(:,isw), &
                         itab, jtab, ttab, utab, casm(:,:,lchnk))
                                    
          ! ----->  parameterized optical properties
          
            do i=1,ncol
            
                ! --- Coefficient xrwet (Eq. 14)
                ! normalize size parameter
                 xrwet(i,k,m) = (2*log(rwet_mode(i,k,m,lchnk))-xrmin-xrmax)/(xrmax-xrmin)
                 ...
                 ! radsurf(i,k) = 0.5_r8*dgnumwet(i,k)*explnsigma ! --- original from modal_size_parameters
                 ! --- assumption that dgnumet = 2*rwet_mode
                 radsurf(i,k,lchnk) = 0.5_r8*2*rwet_mode(i,k,m,lchnk)*explnsigma
                 logradsurf(i,k,lchnk) = log(radsurf(i,k,lchnk))

                 !------> normalize size parameter
                 xrad(i) = max(logradsurf(i,k,lchnk),xrmin)
               ...
               if (logradsurf(i,k,lchnk) .le. xrmax) then
                  pext(i,lchnk) = 0.5_r8*cext(i,1,lchnk) ! parameterized specific extinction (m2/kg)
                  do nc = 2, ncoef
                     pext(i,lchnk) = pext(i,lchnk) + cheb(nc,i,k)*cext(i,nc,lchnk)                           
                  enddo
                  pext(i,lchnk) = exp(pext(i,lchnk))
               else
                  pext(i,lchnk) = 1.5_r8/(radsurf(i,k,lchnk)*rhoh2o) ! geometric optics
               endif


             ! -----> convert from m2/kg water to m2/kg aerosol
             
               !specpext(i) = pext(i,lchnk)
               
!                pext(i,lchnk) = pext(i,lchnk)*wetvol(i)*rhoh2o
               pext(i,lchnk) = pext(i,lchnk)*vmr_wet(i,k,m,lchnk)*rhoh2o ! parameterized specific extinction (m2/kg)
               pabs(i,lchnk) = 0.5_r8*cabs(i,1,lchnk)                    ! parameterized specific absorption (m2/kg)
               pasm(i,lchnk) = 0.5_r8*casm(i,1,lchnk)                    ! parameterized asymmetry factor
               
               do nc = 2, ncoef
                  pabs(i,lchnk) = pabs(i,lchnk) + cheb(nc,i,k)*cabs(i,nc,lchnk)
                  pasm(i,lchnk) = pasm(i,lchnk) + cheb(nc,i,k)*casm(i,nc,lchnk)
               enddo
               
!                pabs(i,lchnk) = pabs(i,lchnk)*wetvol(i)*rhoh2
               pabs(i,lchnk) = pabs(i,lchnk)*vmr_wet(i,k,m,lchnk)*rhoh2o
               pabs(i,lchnk) = max(0._r8,pabs(i,lchnk))
               pabs(i,lchnk) = min(pext(i,lchnk),pabs(i,lchnk))

               palb(i,lchnk) = 1._r8-pabs(i,lchnk)/max(pext(i,lchnk),1.e-40_r8) ! parameterized single scattering albedo
               palb(i,lchnk) = 1._r8-pabs(i,lchnk)/max(pext(i,lchnk),1.e-40_r8) ! --- this is commented because looks duplicated in original code

               dopaer(i,k,m,lchnk) = pext(i,lchnk)*mass(i,k)              ! ---! aerosol optical depth in layer 
               Computed_AOD(i,lchnk) =  Computed_AOD(i,lchnk) + dopaer(i,k,m,lchnk)                          
   ...
   return
  end subroutine ! aerosol_sw_properties
  !================================================================  
  ...
  subroutine qaer_target_mam4(lchnk,ncol)  
   ...
   integer, parameter :: nmde = 4
      ...
      do kcol =1,ncol
      	  if 	((Computed_AOD(kcol,lchnk) .le. 0.01) .or. (Target_AOD(kcol,lchnk) .lt. 0)) then          	  
      	  		AOD_ratio(kcol,lchnk) = 1
!       	  		write(iulog,*) '---> Computed_AOD is less than 0.01'
      	  else      	  	
	      		AOD_ratio(kcol,lchnk) = Target_AOD(kcol,lchnk)/Computed_AOD(kcol,lchnk)
! 	      		AOD_ratio(kcol,lchnk) = 1 !Fred suggestion
! 	      		write(iulog,*) 'Target_AOD= ', Target_AOD(kcol,lchnk)
! 	      		write(iulog,*) 'Computed_AOD= ', Computed_AOD(kcol,lchnk)	      			      		
! 	      		write(iulog,*) 'AOD_ratio= ', AOD_ratio(kcol,lchnk)
	      end if   		
      end do
      
	 ! --- Subroutine created to combine the Model_Qaer for each mode in MAM4
! 	  call Create_Model_Qaer_Mode(ncol,pver,lchnk)	  ! ---- commented Mar-17 does not do anything?       	     	
	  
      do kcol = 1,ncol       
         do l = 1, naer
            Target_Qaer(kcol,:pver,l,lchnk)=AOD_ratio(kcol,lchnk)*Model_Qaer(kcol,:pver,l,lchnk)         
!             Target_Qaer(kcol,:pver,l,lchnk)=2.0*Model_Qaer(kcol,:pver,l,lchnk)
         end do
      end do
