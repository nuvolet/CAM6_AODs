...
public :: modal_aer_opt_readnl, modal_aer_opt_init, modal_aero_sw, modal_aero_lw, get_aodvisdn

! ---- > AODVISdn public for nudging_AODs_qaer_modis_AODVISdn_public.F90
real(r8), allocatable, save :: saved_aodvisdn(:,:)  ! (pcols, nchunks) ! --- commented and moved to 
integer, save :: current_chunk = 0
...
subroutine modal_aer_opt_init()
   ...
   ! real(r8), allocatable, save :: saved_aodvisdn(:, :)  ! (pcols, nchunks) ----- added here to see if do not accumlate each time step
   !----------------------------------------------------------------------------
   ...
   call addfld ('AODVISdn_computed',     horiz_only,  'A','  ','Aerosol optical depth 550 nm, day night computed',              &
                  flag_xyfill=.true.)
   ...
   if (history_aero_optics) then 
      ...
      call add_default ('AODVISdn_computed'       , 1, ' ') ! --- DF
   ...
   do ilist = 1, n_diag
      if (call_list(ilist)) then
         ...
         call addfld ('AODVISdn_computed'//diag(ilist),     horiz_only,  'A','  ',&
              'Aerosol optical depth 550 nm, day night Computed',  flag_xyfill=.true.) ! DF
   ...
   ! Allocate saved AODVISdn array
   if (.not. allocated(saved_aodvisdn)) then
      allocate(saved_aodvisdn(pcols, begchunk:endchunk))
      saved_aodvisdn(:,:) = 0.0_r8
      ! allocate(saved_aodvisdn(pcols))
      ! saved_aodvisdn(:) = 0.0_r8
...
subroutine modal_aero_sw(list_idx, state, pbuf, nnite, idxnite, &
   ...
   ! ---- > AODVISdn public for nudging_AODs_qaer_modis_AODVISdn_public.F90
   ! real(r8) :: AODVISdn_computed(pcols)               ! extinction optical depth
         ...
         do k = top_lev, pver
            ...
            if (savaervis) then
               ...
               do i = 1, ncol
                  ...
                  ! ---- > AODVISdn public for nudging_AODs_qaer_modis_AODVISdn_public.F90
                  saved_aodvisdn(i, lchnk) = saved_aodvisdn(i, lchnk) + dopaer(i)
                  ! saved_aodvisdn(i) = saved_aodvisdn(i) + dopaer(i) ! ---- DF comment
...
!                  if (nerr_dopaer >= nerrmax_dopaer) then
   ...
   call outfld('AODVISdn_computed'//diag(list_idx),   saved_aodvisdn,  pcols, lchnk)
...
subroutine get_aodvisdn(lchnk, ncol, aod_output)
   integer, intent(in) :: lchnk
   integer, intent(in) :: ncol
   ! real(r8), intent(out) :: aod_output(ncol) ! ---- DF suggestion
      real(r8), intent(out) :: aod_output(ncol, lchnk)

   ! ! Check bounds
   ! if (lchnk < begchunk .or. lchnk > endchunk) then
   !    call endrun('get_aodvisdn: chunk index out of bounds')
   ! end if

   ! Return the saved AODVISdn for this chunk
   ! aod_output(1:ncol) = saved_aodvisdn(1:ncol) ! ---- DF suggestion
   ! aod_output(:ncol) = saved_aodvisdn(:ncol) ! ---- DF suggestion
   aod_output(:ncol, lchnk) = saved_aodvisdn(:ncol, lchnk)
