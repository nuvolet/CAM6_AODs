...
subroutine modal_aer_opt_init()
   ...
   call addfld ('AODVISdn_computed',     horiz_only,  'A','  ','Aerosol optical depth 550 nm, computed day night',              &
                  flag_xyfill=.true.) ! introduced to be used in nudging_AODs_qaer_modis_AODVISdn_public.F90, but also output for diagnostics purposes
   ...
   if (history_aero_optics) then 
      ...
      call add_default ('AODVISdn_computed'       , 1, ' ') ! tvmora February 2026
...
subroutine get_aodvisdn(aod_output, ncol, lchnk)
   ...
   aod_output(1:ncol, lchnk) = saved_aodvis(1:ncol, lchnk)
