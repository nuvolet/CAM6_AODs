...
subroutine modal_aero_sw(list_idx, state, pbuf, nnite, idxnite, &
   ...
   ! real(r8) :: saved_aodvisdn(pcols)               ! saved AODVISdn values
...
subroutine get_aodvisdn(lchnk, ncol, aod_output)
   ...
   if (.not. allocated(saved_aodvisdn)) then
      call endrun('get_aodvisdn: saved_aodvisdn not allocated')
   end if

   ! Copy from module-level 2D array
   aod_output(1:ncol) = saved_aodvisdn(1:ncol, lchnk)
