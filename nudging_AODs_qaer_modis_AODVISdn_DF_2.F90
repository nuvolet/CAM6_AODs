...
!   integer          :: Nudge_Beg_Date  ,Nudge_End_Date
  ...
  real(r8),allocatable::AODVISdn_computed     (:,:)  !(pcols,begchunk:endchunk) ! DF2
...
!   real(r8),allocatable:: tauxar(:,:,:)      ! pcols,pver,begchunk:endchunk
  ...
  ! real(r8),allocatable:: AODVISdn_computed(:,:)      ! pcols,begchunk:endchunk  ---> DF suggestion
...
!    call addfld ('dopaer',   (/ 'lev ', 'nmde'/), 'A','  ','Aerosol optical depth in layer',  flag_xyfill=.true.,fill_value=-999.0_r8) 
   call addfld ('AODVISdn_computed',   horiz_only, 'A','  ','AODVISdn computed',  flag_xyfill=.true.,fill_value=-999.0_r8)    ! ---- DF suggestion              
...
!       call outfld('palb',          palb(:,lchnk),    pcols, lchnk)           
      ...
      call outfld('AODVISdn_computed',  AODVISdn_computed(:,lchnk),    pcols, lchnk)      ! ---- DF suggestion       
...
!   Subroutines deleted in order to reuse AODVISdn from modela_aer_opt.F90
!   relative_humidty
...
!   aerosol_sw_properties
  ...
  subroutine qaer_target_mam4(lchnk,ncol)  
   ...
   integer :: l, kcol
   integer, parameter :: nmde = 4
  !  real(r8) :: AODVISdn_computed(ncol) ! ---- DF suggestion
   real(r8) :: AODVISdn_computed(ncol,lchnk) ! --- added lchnk for outfield check
  !  AODVISdn_computed(:pcols,lchnk)=0_r8     
   AODVISdn_computed(:,lchnk)=0_r8     
  !  call get_aodvisdn(lchnk, ncol, AODVISdn_computed)
  ...
  ! Compute the ratio between MODIS AOD and computed from RTM
    ...
    do kcol =1,ncol
        ...
        !  AODVISdn_computed
        ! if 	((AODVISdn_computed(kcol) .le. 0.01) .or. (Target_AOD(kcol,lchnk) .lt. 0)) then ! ---- DF suggestion          	  
        if 	((AODVISdn_computed(kcol,lchnk) .le. 0.01) .or. (Target_AOD(kcol,lchnk) .lt. 0)) then          	  
          AOD_ratio(kcol,lchnk) = 1
        else      	  	
          ! AOD_ratio(kcol,lchnk) = Target_AOD(kcol,lchnk)/AODVISdn_computed(kcol) ! ---- DF suggestion
