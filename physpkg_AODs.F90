...
!   use nudging_AODs,       only: Nudge_Model_AOD, nudging_init_AOD ! ---- >06/17/24
...
!=======================================================================
  ...
  subroutine phys_init( phys_state, phys_tend, pbuf2d, cam_in, cam_out )
    ...
    use nudging_AODs,       only: Nudge_Model_AOD, nudging_init_AOD   
    ...
    !--------------------------------
    write(iulog,*) '|------ physpkg.F90 ----- > Initialize Nudging Parameters'
    
    if(Nudge_Model) call nudging_init
    write(iulog,*) '|------ physpkg.F90 ----- > Calling nudging_init in physpkg.F90'
    write(iulog,*) '|------ physpkg.F90 ----- > Nudge_Model: ', Nudge_Model        
    
    if(Nudge_Model_AOD) call nudging_init_AOD ! --- Here we force to call nudging_init_AOD
    write(iulog,*) '|------ physpkg.F90 ----- > Calling nudging_init_AOD'
    write(iulog,*) '|------ physpkg.F90 ----- > Nudge_Model_AOD: ', Nudge_Model_AOD    
!     call nudging_init_AOD
!     write(iulog,*) '|------ physpkg.F90 ----- > nudging_init_AOD has been forced to be call !!!'
...
!$OMP PARALLEL DO PRIVATE (C, NCOL, phys_buffer_chunk)
  ...
  subroutine tphysac (ztodt,   cam_in,  &
    ...
    use nudging_AODs,      only: Nudge_Model_AOD,Nudge_AOD_ON,nudging_timestep_tend_AOD
   ...
   if (carma_do_aerosol) then
    ...
    if((Nudge_Model).and.(Nudge_ON)) then
	  write(iulog,*), '|------------------ > Update Nudging values - Calling nudging_timestep_tend in physpkg.F90'
      call nudging_timestep_tend(state,ptend)
      call physics_update(state,ptend,ztodt,tend)
      call check_energy_chng(state, tend, "nudging", nstep, ztodt, zero, zero, zero, zero)
    endif

    ! Update Nudging values, if needed - for AOD assimilation
    !----------------------------------
    if((Nudge_Model_AOD).and.(Nudge_AOD_ON)) then
	  write(iulog,*), '|------------------ > Update Nudging AOD values - Calling nudging_timestep_tend_AOD in physpkg.F90'    
      call nudging_timestep_tend_AOD(state,ptend)
      call physics_update(state,ptend,ztodt,tend)
      call check_energy_chng(state, tend, "nudging_AODs", nstep, ztodt, zero, zero, zero, zero)
    endif
...
!-----------------------------------------------------------------------------------
  ...
  use nudging_AODs,       only: Nudge_Model_AOD, nudging_timestep_init_AOD  
  ...
  ! Update Nudging values, if needed
  !----------------------------------
  write(iulog,*) '|------ physpkg.F90 ----- > call nudging_timestep_init(phys_state)'
  if(Nudge_Model) call nudging_timestep_init(phys_state)

  ! Update Nudging values, if needed -> AOD assimilation
  !----------------------------------
  write(iulog,*) '|------ physpkg.F90 ----- > call nudging_timestep_init_AOD(phys_state)'
  if(Nudge_Model_AOD) call nudging_timestep_init_AOD(phys_state)
